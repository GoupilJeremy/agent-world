#!/bin/bash
# Script pour créer les labels GitHub pour Agent World
# Exécution : bash scripts/create_github_labels.sh
...
create_label() {
    ...
    if gh api -X POST /repos/$REPO/labels -f "name=$name" -f "description=$description" -f "color=$color" >/dev/null 2>&1; then

...
class IntegrationManager:
        ...
        for integration_type in IntegrationType:
            ...
            if adapter_class:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to get metadata for {integration_type.value}: {e}"
                    )
                    supported.append(
                        {
                            "type": integration_type.value,
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_actions = sum(c.usage_count for c in self._configurations.values())
        total_errors = sum(c.error_count for c in self._configurations.values())
        ...
        return {
            ...
            "success_rate": (
                total_actions / (total_actions + total_errors)
                if (total_actions + total_errors) > 0
                else 0
            ),

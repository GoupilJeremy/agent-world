...
class GoogleDriveIntegrationAdapter(BaseIntegrationAdapter):
    ...
    def _get_auth_headers(self) -> Dict[str, str]:
        ...
        if not self.config or not self.config.credentials:
            raise AuthenticationError(
                "No credentials configured for Google Drive",
                self.type
            )
        ...
        if credentials.access_token:
            ...
            raise AuthenticationError(
                "No valid credentials found for Google Drive",
                self.type
            )
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                if response.status_code == 401:
                    raise AuthenticationError("Invalid or expired Google token", self.type)
                    ...
                    if error_reason == "rateLimitExceeded":
                        raise ConnectionError("Google Drive rate limit exceeded", self.type)
                    elif error_reason == "dailyLimitExceeded":
                        raise ConnectionError("Google Drive daily quota exceeded", self.type)
                    elif error_reason == "userRateLimitExceeded":
                        raise ConnectionError("Google Drive user rate limit exceeded", self.type)
                    else:
                        raise ConnectionError(f"Google Drive permission denied: {error_reason}", self.type)
                elif response.status_code == 404:
                    raise ConnectionError(f"Google Drive resource not found: {url}", self.type)
                else:
            ...
            if response is not None:
                ...
                return IntegrationResult(
                    ...
                    data={
                        ...
                        "content_type": mimetypes.guess_type(metadata.get("name", ""))[0],
                        "metadata": metadata,
                    },
                )
            else:
    ...
    def _update_file_metadata(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            valid_fields = {
                "name", "description", "mimeType", "starred", "trashed"
            }
            update_data = {
    ...
    def _add_permission(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            params = {
                "sendNotificationEmail": str(payload.get("send_notification", False)).lower(),
            }

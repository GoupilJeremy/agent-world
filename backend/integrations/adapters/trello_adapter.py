...
class TrelloIntegrationAdapter(BaseIntegrationAdapter):
    ...
    def _get_auth_params(self) -> Dict[str, str]:
        ...
        if not self.config or not self.config.credentials:
            raise AuthenticationError("No credentials configured for Trello", self.type)
        ...
        # Méthode 2: OAuth2 (access_token)
            ...
            raise AuthenticationError(
                "No valid credentials found for Trello", self.type
            )
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                if response.status_code == 401:
                    raise AuthenticationError(
                        "Invalid or expired Trello credentials", self.type
                    )
                elif response.status_code == 403:
                    raise ConnectionError("Trello permission denied", self.type)
                elif response.status_code == 404:
                    raise ConnectionError(
                        f"Trello resource not found: {endpoint}", self.type
                    )
                elif response.status_code == 429:
                    raise ConnectionError("Trello API rate limited", self.type)
                else:
                    raise ConnectionError(
                        f"Trello API error: {response.status_code}", self.type
                    )
    ...
    def get_authentication_url(
        self, state: Optional[str] = None, scope: Optional[str] = None
    ) -> str:
        """
    ...
    def refresh_token(self, refresh_token: str) -> IntegrationCredentials:
        ...
        raise NotImplementedError(
            "Trello OAuth1 tokens cannot be refreshed. " "Please re-authenticate."
        )
        ...
        try:
            ...
            if label_id:
                ...
                response = self._make_request(
                    "POST", endpoint, json={"value": label_id}
                )
    ...
    def _add_member_to_card(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            response = self._make_request("POST", endpoint, json={"value": member_id})

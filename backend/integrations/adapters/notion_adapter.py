...
class NotionIntegrationAdapter(BaseIntegrationAdapter):
    ...
    OAUTH_CONFIG = OAuthConfig(
        ...
        scope=["read:user", "read:page", "write:page", "read:database", "write:database"],
    )
    ...
    def _get_auth_headers(self) -> Dict[str, str]:
        ...
        if not self.config or not self.config.credentials:
            raise AuthenticationError(
                "No credentials configured for Notion",
                self.type
            )
        ...
        if credentials.access_token:
            ...
            raise AuthenticationError(
                "No valid credentials found for Notion",
                self.type
            )
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                if response.status_code == 401:
                    raise AuthenticationError("Invalid or expired Notion token", self.type)
                elif response.status_code == 403:
                    raise ConnectionError("Notion rate limit exceeded", self.type)
                elif response.status_code == 404:
                    raise ConnectionError(f"Notion resource not found: {url}", self.type)
    ...
    def test_connection(self) -> IntegrationResult:
        ...
        try:
            ...
            if user_data and isinstance(user_data, dict) and user_data.get("object") == "user":
    ...
    def _delete_page(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            if response and response.get("object") == "page" and response.get("archived") is True:
                return IntegrationResult(
                    success=True,
    ...
    def _sync_database(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            sync_direction = payload.get("direction", "to_notion")  # to_notion, from_notion, bidirectional
                ...
                if get_page_result.success:
                    return IntegrationResult(
                        ...
                        data={
                            ...
                            "synced_properties": list(agent_data.get("properties", {}).keys()),
                        },
                    )
                else:

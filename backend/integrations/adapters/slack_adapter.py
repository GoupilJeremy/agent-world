...
class SlackIntegrationAdapter(BaseIntegrationAdapter):
    ...
    def _get_auth_headers(self) -> Dict[str, str]:
        ...
        raise AuthenticationError("No valid Slack token found", self.type)
            ...
            if response.status_code >= 400:
                ...
                try:
                    ...
                    if not response_json.get("ok"):
                        error_data["slack_error"] = response_json.get(
                            "error", "Unknown error"
                        )
                except Exception:
                ...
                if response.status_code == 401:
                    raise AuthenticationError(
                        "Invalid or expired Slack token", self.type
                    )
                elif response.status_code == 429:
                    raise ConnectionError("Slack API rate limit exceeded", self.type)
                else:
                    error_msg = (
                        response_json.get("error", response.text)
                        if response_json
                        else response.text
                    )
                    raise ConnectionError(f"Slack API error: {error_msg}", self.type)
    ...
    def exchange_code_for_token(self, code: str) -> IntegrationCredentials:
        ...
        try:
            ...
            access_token = token_data.get("authed_user", {}).get(
                "access_token"
            ...
            bot_token = token_data.get(
                "access_token"
            )  # Le bot token est dans access_token
    ...
    def _create_channel(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            result = self._make_request(
                "POST", "conversations.create", json=channel_data
            )
    ...
    def _join_channel(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            result = self._make_request(
                "POST", "conversations.join", json={"channel": channel}
            )
    ...
    def _open_conversation(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            ...
            result = self._make_request(
                "POST", "conversations.open", json={"users": users}
            )

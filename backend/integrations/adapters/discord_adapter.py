...
class DiscordIntegrationAdapter(BaseIntegrationAdapter):
        ...
        try:
            ...
            if response.status_code >= 400:
                ...
                try:
                    ...
                    error_data["discord_error"] = response_json.get("message", "Unknown error")
                ...
                if response.status_code == 401:
                    raise AuthenticationError("Invalid or expired Discord token", self.type)
        ...
        try:
            ...
            if result and isinstance(result, dict):
                ...
                return IntegrationResult(
                    ...
                    data={
                        "user_id": user_id,
                        "username": f"{username}#{discriminator}" if username and discriminator else username,
    ...
    def _get_guild_info(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            guild_id = payload.get("guild_id")
            with_counts = payload.get("with_counts", True)  # Inclure les comptes de membres
    ...
    def _add_reaction(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            channel_id = payload.get("channel_id")
            message_id = payload.get("message_id")
            emoji = payload.get("emoji")  # Peut être un emoji Unicode ou un emoji personnalisé (format: name:id)
    ...
    def _create_slash_command(self, payload: Dict[str, Any]) -> IntegrationResult:
        ...
        try:
            application_id = payload.get("application_id")  # ID de l'application Discord
            name = payload.get("name")

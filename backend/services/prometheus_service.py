...
if hasattr(multiprocess, "MultiprocessMode") and getattr(
    multiprocess.MultiprocessMode, "needs_prometheus", False
):
...
class PrometheusService:
    ...
    def _get_or_create_counter(self, name, documentation, labelnames=()):
        ...
        if existing is not None:
            return existing
        return Counter(
            name, documentation, list(labelnames), registry=PROMETHEUS_REGISTRY
        )

    def _get_or_create_histogram(
        self, name, documentation, labelnames=(), buckets=None
    ):
        existing = PROMETHEUS_REGISTRY._names_to_collectors.get(name)
        if existing is not None:
            return existing
    ...
    def track_agent_execution(
        self, agent_id: str, status: str = "success", duration: float = None
    ):
        """Track l'exécution d'un agent."""
        self.agents_executed_total.labels(agent_id=agent_id, status=status).inc()

        if duration is not None:
            self.agents_execution_duration_seconds.labels(agent_id=agent_id).observe(
                duration
            )

    def track_file_generation(
        self, file_type: str, agent_id: str = "unknown", size_bytes: int = 0
    ):

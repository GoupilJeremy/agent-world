...
class CompressionService:
    ...
    def _validate_compression_level(self, level: int) -> int:
        ...
        if not isinstance(level, int) or level < 0 or level > 9:
            raise ValueError(
                "Le niveau de compression doit être un entier entre 0 et 9"
            )
        return level
    ...
    def compress_file(
        ...
        try:
            ...
            raise CompressionError(
                f"Échec de la compression: {e}", str(source_path)
            ) from e
    ...
    def compress_directory(
        ...
        try:
            ...
            if dest_path.exists():
                dest_path.unlink()
            raise CompressionError(
                f"Échec de la compression du dossier: {e}", str(source_dir)
            ) from e
    ...
    def _add_directory_to_zip(
        ...
        for item in directory.iterdir():
            ...
            if include_patterns:
                if not any(
                    item.name.lower().endswith(ext.lower()) for ext in include_patterns
                ):
                    continue

            if exclude_patterns:
                if any(
                    item.name.lower().endswith(ext.lower()) for ext in exclude_patterns
                ):
                    continue
        ...
        try:
            ...
            if dest_path.exists():
                if dest_path.is_dir():
                    ...
                    dest_path.unlink()
            raise CompressionError(
                f"Échec de la décompression: {e}", str(source_path)
            ) from e

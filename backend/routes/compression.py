...
class CompressionStatsResource(Resource):
    ...
    def get(self):
        ...
        try:
            compression_service: CompressionService = current_app.extensions[
                "compression_service"
            ]
            ...
            parser.add_argument(
                ...
                type=str,
                required=True,
                help="Chemin du fichier original",
            )
            parser.add_argument(
                "compressed_path",
                type=str,
                required=True,
                help="Chemin du fichier compressé",
            )
            args = parser.parse_args()

            stats = compression_service.get_compression_stats(
                args["original_path"], args["compressed_path"]
            )
    ...
    def post(self):
        ...
        try:
            ...
            if not compression_service.is_compressed(file.filename):
                return {
                    "success": False,
                    "error": "Le fichier n'est pas compressé",
                }, 400
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if decompressed_path.is_dir():
                    ...
                    files_list = [
                        f.name for f in decompressed_path.iterdir() if f.is_file()
                    ]
                    return {
                        "success": True,
                        "data": {
    ...
    def get(self):
        ...
        try:
            ...
            current_app.logger.error(
                f"Erreur lors de la récupération de la configuration: {e}"
            )
            return {"success": False, "error": str(e)}, 500
...
class CheckCompressionResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            parser = reqparse.RequestParser()
            parser.add_argument(
                "path", type=str, required=True, help="Chemin du fichier à vérifier"
            )
            args = parser.parse_args()
            ...
            return {
                "success": True,
                "data": {"path": args["path"], "is_compressed": is_compressed},
            }, 200

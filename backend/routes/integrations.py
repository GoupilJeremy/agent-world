...
class IntegrationListResource(Resource):
    ...
    def post(self):
        ...
        config = manager.create_integration(
            ...
            credentials=IntegrationCredentials(**data.get("credentials", {})) if data.get("credentials") else None,
...
class IntegrationActivateResource(Resource):
    ...
    def post(self, integration_id, action):
        ...
        if action == "activate":
            ...
            message = "Integration activated" if success else "Failed to activate integration"
        elif action == "deactivate":
            success = manager.deactivate_integration(int(integration_id))
            message = "Integration deactivated" if success else "Failed to deactivate integration"
        else:
            return {"error": f"Invalid action: {action}. Must be 'activate' or 'deactivate'"}, 400
...
class OAuthCallbackResource(Resource):
    ...
    def get(self, integration_type):
        ...
        return {
            ...
            "credentials": {
                "access_token": credentials.access_token[:10] + "..." if credentials.access_token else None,
                "token_type": "Bearer",
                "expires_in": (credentials.token_expiry.timestamp() - datetime.utcnow().timestamp()) if credentials.token_expiry else None,
            },
        }
...
def register_integration_resources(api):
    ...
    api.add_resource(IntegrationActionsResource, "/integrations/<int:integration_id>/actions")
    ...
    api.add_resource(IntegrationMetadataResource, "/integrations/<string:integration_type>/metadata")
    api.add_resource(
    ...
    # OAuth2
    api.add_resource(OAuthAuthorizationResource, "/integrations/<string:integration_type>/oauth/authorize")
    api.add_resource(OAuthCallbackResource, "/integrations/<string:integration_type>/oauth/callback")
    ...
    api.add_resource(GitHubIntegrationResource, "/integrations/github/<int:integration_id>")

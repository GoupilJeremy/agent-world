...
Examples:
  ...
  agent run 1 --input "Hello!"                         Run an agent
    ...
    parser.add_argument(
        '--version', '-v',
        action='version',
        version='Agent World CLI v0.1.0'
    )
    ...
    create_parser.add_argument(
        '--name', '-n',
        required=True,
        help='Name of the agent (required)'
    )
    create_parser.add_argument(
        '--model', '-m',
    ...
    list_parser = subparsers.add_parser(
        ...
        description='List all AI agents in the database'
    )
    list_parser.add_argument(
        '--all',
        action='store_true',
        help='Show all agents including inactive ones'
    )
    list_parser.add_argument(
        '--limit', '-l',
        type=int,
    ...
    list_parser.add_argument(
        '--search', '-s',
        help='Search agents by name or description'
    )
    ...
    show_parser.add_argument(
        'id',
        type=int,
        help='ID of the agent to show'
    )
    show_parser.add_argument(
        '--executions', '-e',
        action='store_true',
        help='Show agent executions'
    )
    show_parser.add_argument(
        '--stats',
        action='store_true',
    ...
    update_parser.add_argument(
        '--model', '-m',
        help='New AI model for the agent'
    )
    update_parser.add_argument(
        '--description', '-d',
    ...
    update_parser.add_argument(
        '--config', '-c',
        type=json.loads,
        help='New JSON configuration for the agent'
    )
    update_parser.add_argument(
        '--active', '-a',
        type=bool,
        help='Whether the agent should be active'
    )
    ...
    run_parser.add_argument(
        '--input', '-i',
        required=True,
        help='Input text for the agent (required)'
    )
    run_parser.add_argument(
        '--model', '-m',
        help='Override the agent model'
    )
    run_parser.add_argument(
        '--output', '-o',
...
class AgentWorldCLI:
    ...
    def handle_list(self, args) -> int:
        ...
        try:
            if args.search:
                ...
                agents = [a.to_dict() for a in self.agent_service.get_all_agents(only_active=False)]
            else:
                agents = [a.to_dict() for a in self.agent_service.get_all_agents(only_active=True)]
    ...
    def handle_show(self, args) -> int:
        ...
        try:
            ...
            if args.stats:
                ...
                if executions:
                    self.print_table(executions, title=f"📋 Executions for Agent {agent.name}")
                else:
    ...
    def handle_delete(self, args) -> int:
        ...
        try:
            ...
            if not args.force:
                response = input(f"🗑️  Are you sure you want to delete agent '{agent.name}'? (y/N): ")
    ...
    def print_table(self, data: List[Dict[str, Any]], title: str = None) -> None:
        ...
        display_headers = [h for h in headers if h not in ['description', 'configuration', 'input_data', 'output_data']]
